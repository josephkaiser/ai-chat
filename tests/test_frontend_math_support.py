...
class FrontendMathSupportTests(unittest.TestCase):
    def test_generated_javascript_is_built_from_typescript_source(self):
        ...
        self.assertIn("const paragraphLines: string[] = [];", ts)
        self.assertIn("const flushParagraph = () => {", ts)
        self.assertIn('blocks.push(`<p>${renderInlineMarkdown(paragraphLines.join("\\n"))}</p>`);', ts)
    ...
    def test_repo_includes_local_frontend_build_entrypoint(self):
        ...
        self.assertIn("MODEL_14B_NAME=Qwen/Qwen3-14B-AWQ", model_defaults)
        self.assertIn("MODEL_14B_GPU_MEMORY_UTILIZATION=", model_defaults)
        self.assertIn("MODEL_14B_MAX_MODEL_LEN=", model_defaults)
        self.assertIn("MODEL_14B_ENABLE_PREFIX_CACHING=1", model_defaults)
        self.assertIn("MODEL_14B_MAX_MODEL_LEN=4096", model_override_sample)
