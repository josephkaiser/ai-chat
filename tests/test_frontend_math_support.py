...
class FrontendMathSupportTests(unittest.TestCase):
    def test_generated_javascript_is_built_from_typescript_source(self):
        ...
        self.assertIn("type RenderMarkdownOptions =", ts)
        self.assertIn("function renderMarkdownBlocks(blocks: MarkdownBlock[], options: RenderMarkdownOptions = {}): string", ts)
        self.assertIn("function renderAssistantRichText(raw: string, messageIndex: number): string", ts)
