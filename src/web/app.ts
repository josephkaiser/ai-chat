...
const taskBoardCache = new Map<string, string>();
...
function isHiddenWorkspacePath(path: string): boolean {
    return resolveArtifactReferencePath(path)
        .split("/")
        .filter(Boolean)
...
type RenderMarkdownOptions = {
    reasoningMessageIndex?: number;
};

function markdownBlockPlainText(block: MarkdownBlock): string {
...
function shouldShowReasoningButtonForBlock(block: MarkdownBlock): boolean {
...
function renderMarkdownBlocks(blocks: MarkdownBlock[], options: RenderMarkdownOptions = {}): string {
...
function renderAssistantRichText(raw: string, messageIndex: number): string {
    const singleFence = unwrapSingleFence(raw);
    if (singleFence && ["markdown", "md"].includes(singleFence.language)) {
...
function reasoningScopeKeywords(text: string): string[] {
    const words = cleanConversationText(text)
        .toLowerCase()
        .split(/\s+/)
...
function scopeRelevanceScore(scopeText: string, candidate: string): number {
...
function reasoningLabelForPhase(note: ReasoningNote): string {
    const pieces = [String(note.phase || "reasoning").trim(), String(note.step_label || "").trim()].filter(Boolean);
    return pieces.length ? pieces.join(" · ") : "Reasoning";
}

function parseTaskBoardSections(raw: string): string[] {
...
function chooseReasoningSections(scopeText: string, taskBoardText: string): string[] {
    const sections = parseTaskBoardSections(taskBoardText);
    if (!sections.length) return [];
    const ranked = sections
        .map((section) => ({ section, score: scopeRelevanceScore(scopeText, section) }))
...
function renderReasoningNotes(notes: ReasoningNote[]): string {
    if (!notes.length) {
...
function renderReasoningTaskBoard(scopeText: string, taskBoardText: string): string {
    ...
    return sections
        .map((section) => `<div class="reasoning-card">${renderRichText(section)}</div>`)
        .join("");
}

async function loadReasoningTaskBoard(path: string): Promise<string> {
    ...
    try {
        const payload = await fetchJson<WorkspaceFilePayload>(workspaceFileApiUrl(path, String(Date.now())));
...
function closeReasoningOverlay(): void {
...
async function openReasoningOverlay(messageIndex: number, blockIndex: number): Promise<void> {
    ...
    const rankedNotes = [...(message.reasoning_notes || [])]
        .map((note) => ({
            note,
    ...
    const taskBoardPath = [...parsedArtifacts.hiddenArtifactPaths]
        .find((path) => /(?:^|\/)\.ai\/task-board\.md$/i.test(path))
        || ".ai/task-board.md";
    ...
    const cards = reasoningContent.querySelectorAll<HTMLElement>(".reasoning-card");
...
function bindReasoningButtons(root: ParentNode): void {
    root.querySelectorAll<HTMLButtonElement>("[data-reasoning-message-index]").forEach((button) => {
...
function renderMessages(): void {
    ...
    const messagesHtml = state.messages.map((message, index) => {
        ...
        const parsedArtifacts = message.role === "assistant"
            ...
            : { artifactPaths: [], hiddenArtifactPaths: [], displayContent: message.content || "", materializedBody: "" };
        ...
        const bodyHtml = parsedArtifacts.displayContent
            ? `<div class="message-content rich-text">${message.role === "assistant" ? renderAssistantRichText(parsedArtifacts.displayContent || "", index) : renderRichText(parsedArtifacts.displayContent || "")}</div>`
            : "";
...
function recordReasoningNote(event: ChatEvent): void {
    ...
    assistantMessage.reasoning_notes = [...notes, note].slice(-18);
}
